...
public class BattleInputHandler {
	...
	private final int worldBorderLeftOffset = 25;
	private final int worldBorderRightOffset = 110;
	
	public BattleInputHandler() {
		
	}
	
	public BattleInputHandler(BattleScreen b) {
		parent = b;
	}
	
	public void handleUnitControl(GameContainer container, int delta, Ground ground, Unit unit, int worldWidth) {
		...
		if ((container.getInput().isKeyDown(Input.KEY_LEFT)||
			...
			unit.body.setLinearVelocity(new Vec2 (-100, unit.body.getLinearVelocity().y));
			x -= delta*0.1;
	...
	public BattleMouseClickResult handleMouseClick(GameContainer container, Army currentArmy, int screenOffsetX, int screenOffsetY, BattleButtons battleButtons) {
		...
		// battle Buttons
		if (mouseClicked) {
			int num = battleButtons.checkMouseIn((int)mouseX, (int)mouseY);
			if (num != 0) {
				return new BattleMouseClickResult(num, false, 0, mouseClicked, mouseX, mouseY);
			}
		}
		
		boolean unitSelected = false;
		int numOfOverlap = 0;
		float x = (float)(mouseX+screenOffsetX);
		float y = (float)(mouseY+screenOffsetY);
		if (mouseClicked) {
		...
		return new BattleMouseClickResult(0, unitSelected, currentUnitNumber, mouseClicked, mouseX, mouseY);
	}
