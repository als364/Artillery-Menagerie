...
public class MSTerrain
{
        ...
        public MSTerrain(org.jbox2d.dynamics.World world, AABB area)
        {
            World = world;
        ...
        public void Initialize()
        {
            ...
            for(int i = 0; i < _xnum; i++){
            	for(int j = 0; j < _ynum; j++){
            		_bodyMap[i][j] = new ArrayList<Body>();
            	}
            }

            // make sure to mark the dirty area to an infinitely small box
            _dirtyArea = new AABB(new Vec2(Float.MAX_VALUE, Float.MAX_VALUE), new Vec2(Float.MIN_VALUE, Float.MIN_VALUE));
        }
        ...
        public void ApplyTexture(Image texture, Vec2 position, int alpha)
        {
        ...
        /// </summary>
        /// <param name="position">Top left position of the texture relative to the terrain.</param>
        public void ApplyData(byte[][] data, Vec2 position)
        {
        ...
        public void ModifyTerrain(Vec2 location, byte value)
        {
            // find local position
            // make position local to map space
            //Vec2 p = location.sub(_topLeft);
        	Vec2 p = location;
        ...
        public static Vertices CollinearSimplify(Vertices vertices, float collinearityTolerance)
        {
            ...
            for (int i = 0; i < vertices.size(); i++)
            {
                ...
                Vec2 prev = vertices.get(prevId);
                Vec2 current = vertices.get(i);
                Vec2 next = vertices.get(nextId);
        ...
        private void GenerateTerrain(int gx, int gy)
        {
            ...
            List<Vertices> polys = MarchingSquares.DetectSquares(new AABB(new Vec2(ax, ay), new Vec2(ax + CellSize, ay + CellSize)), SubCellSize, SubCellSize, _terrainMap, Iterations, true);
            if (polys.size() == 0) return;

            _bodyMap[gx][ gy] = new ArrayList<Body>();

            // create the scale vector
            Vec2 scale = new Vec2(1f / PointsPerUnit, 1f / -PointsPerUnit);
            ...
            for (Vertices item : polys)
            {
                ...
                for (Vertices poly : decompPolys)
                {
                    if (poly.size() > 2){
                    	...
                    	Vec2 pos = new Vec2(ax + CellSize/2, ay + CellSize/2);
                    	...
                    	for(int i = 0; i < arr.length; i++){
                    		...
                    		//arr[i] = arr[i].add(new Vec2(2*(CellSize/2 - arr[i].x), 2*(CellSize/2 - arr[i].y)));
                    	}
                    	pos = diff;
                    	...
                    	body.m_fixtureList.m_friction = 0.3f;
